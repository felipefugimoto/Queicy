            ...
            <form v-if="appointmentCreated" @submit.prevent="submitServices">
                ...
                <div class="service-section">
                    <div v-for="(service, index) in appointment.services" :key="index" class="service-item row">
                        <div class="col-6">
                            <select v-model="service.servico_id">
                        ...
                        <div class="col-3 service-actions">
                            <button class="delete-button" @click="removeService(index)"><i class="bi bi-trash"></i></button>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        calculateServiceValue(index) {
            ...
            const service = this.servicos.find(s => s.id === selectedService.servico_id);
            if (service) {
                selectedService.value = service.preco.replace('R$', '').trim();
            }
        },
        calculateTotal() {
            let total = 0;
        ...
        async submitServices() {
            try {
                const servicesData = this.appointment.services.map(service => ({
                    agendamento_id: this.appointmentId,
                    servico_id: service.servico_id,
                    cliente: this.cliente
                }));
                await axios.post(`http://127.0.0.1:8000/api/agendamento/${this.appointmentId}/services`, servicesData);
            } catch (error) {
